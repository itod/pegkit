...
@interface TDTokenizerTest : XCTestCase {
    PKTokenizer *t;
    NSString *s;
}
...
- (void)testStuffWithFastEnumeration {
    ...
    for (PKToken *tok in t) {
        ...
        NSString *actual = [NSString stringWithFormat:@"(%@) (%.1f) : %@", tok.stringValue, tok.doubleValue, [tok debugDescription]];
        //NSLog(@"%@", actual);
...
- (void)testDotOne {
    ...
    XCTAssertEqualWithAccuracy(0.999, tok.doubleValue, 0.01, @"");
    TDTrue(tok.isNumber);
